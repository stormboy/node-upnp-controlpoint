 ...
 */
WemoControllee.prototype.change = function(value) {
	var v = value ? 1 : 0;
	var args = { BinaryState : v };
	this.eventService.callAction("SetBinaryState", args, function(err, buf) {
		if (err) {
			console.log("got SOAP err: " + err + " => " + buf);
		}
		else {
			if (TRACE && DETAIL) {
...
var WemoSensor = function(device) {
	...
	for (name in this.device.services) {
		var service = this.device.services[name];
		console.log("sensor service: " + JSON.stringify(service.serviceType));
		if (service.serviceType == SERVICE_TYPE_BasicEvent) {
			self.eventService = service;
			// break;
		}
